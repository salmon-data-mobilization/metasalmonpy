...
and nothing is kept from one call to the next. The degraded case also checks
its lasting outage in a second shape, the empty answer ``find_terms()`` gives
when its only source fails, beside R's one-candidate answer.
...
def test_a_degraded_answer_is_never_served_to_another_target(monkeypatch):
    ...
    # An outage that lasts: every target searches for itself, nothing is
    # shared, and each target keeps the one candidate its own degraded answer
    # gave, as in R.
    outage_fn, outage_keys = _counting_search(lambda n: "partial")
    outage = _suggest(outage_fn)
    reference, reference_keys = _per_target_reference(monkeypatch, lambda n: "partial")
    ...
    outage_iris = outage.attrs["semantic_suggestions"]["iri"]
    assert len(outage_iris) == len(outage_keys)
    assert outage_iris.str.endswith("/partial").all()

    # The same outage in the shape find_terms() gives when its only source
    # fails, answers with no candidates: still searched once per target.
    empty_fn, empty_keys = _counting_search(lambda n: "outage")
    empty = _suggest(empty_fn)
    reference, reference_keys = _per_target_reference(monkeypatch, lambda n: "outage")
    assert len(empty_keys) == len(empty.attrs["semantic_targets"])
    assert collections.Counter(empty_keys) == collections.Counter(reference_keys)
    _assert_same_result(empty, reference)
    assert empty.attrs["semantic_suggestions"].empty
