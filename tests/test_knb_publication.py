"""
Offline parity tests for KNB/DataONE publication (metasalmon 0.1.8 parity).

Fixtures under ``tests/data/knb/`` were generated by running metasalmon at the
**v0.1.8 tag** (R 4.5.2, zip 3.0.1, datapack 1.4.2, emld 0.5.3, ``LC_COLLATE=C``
— see
...
* **Contract-level only** — the SDP ZIP (R allowlists reviewed ``zip``
  versions; Python defines its own :mod:`zipfile` determinism reference) and
  the EML document (libxml2
...
class KnbArchiveTests(unittest.TestCase):
    ...
    def test_archive_is_byte_reproducible_under_the_python_reference(self):
        # R allowlists reviewed zip versions for the same reason: the
        # checksum is an immutable DataONE PID input, so a compressor change
        # must be a reviewed package change rather than an ambient one.
...
# --- expanded representation (0.1.8) ---------------------------------------------


def _expanded_sdp(test_case: unittest.TestCase) -> str:
    """The public fixture SDP, ready for an expanded dry run."""
    return _fixture_sdp(test_case, "sdp-public")


@_REQUIRES_EXTRA
class KnbExpandedRepresentationTests(unittest.TestCase):
    """``representation="expanded"`` publishes the SDP hierarchy, not a ZIP.

    The comparison target is ``tests/data/knb/r/expanded/``, produced by
    metasalmon **v0.1.8** over the same fixture SDP. Every planned object
    except the resource map is byte-identical to R's; the resource map is
    ``ET.canonicalize``-equal (PARITY.md rows 4 and 18).
    """

    def setUp(self):
        self.expected = _expected()["expanded"]

    def test_expanded_plans_preserve_the_sdp_hierarchy_without_a_zip(self):
        ...
        self.assertIsNone(result["sdp_archive_path"])
        objects = result["manifest"]["objects"]
        roles = [obj["role"] for obj in objects]
        paths = [obj["path"] for obj in objects]
        self.assertNotIn("sdp_archive", roles)
        self.assertEqual(roles.count("data"), 1)
        self.assertEqual(roles.count("metadata"), 1)
        self.assertEqual(roles.count("resource_map"), 1)
        self.assertGreater(roles.count("sdp_artifact"), 5)
        for name in (
            "datapackage.json",
        ...
        self.assertFalse([name for name in paths if name.endswith(".zip")])
    ...
    def test_expanded_object_order_pids_and_bytes_match_r(self):
        ...
        self.assertEqual(
            [(obj["role"], obj["path"]) for obj in planned],
            [(obj["role"], obj["path"]) for obj in self.expected["objects"]],
        )
        self.assertEqual(
            result["resource_map_pid"], self.expected["resource_map_pid"]
        )
        by_key = {
            (obj["role"], obj["path"]): obj for obj in self.expected["objects"]
        }
        for obj in planned:
            source = by_key[(obj["role"], obj["path"])]
            with self.subTest(path=obj["path"]):
                self.assertEqual(obj["pid"], source["pid"])
                self.assertEqual(obj["format_id"], source["format_id"])
                if obj["role"] != "resource_map":
                    # Only the ORE document's bytes differ from R's, and only
                    # in formatting; everything else is byte-identical.
                    self.assertEqual(obj["sha256"], source["sha256"])
                    self.assertEqual(obj["size"], source["size"])
    ...
    def test_expanded_ore_is_structurally_identical_to_r(self):
        ...
        self.assertEqual(
            ...
            _canonical_file(os.path.join(_R_DIR, "expanded", "resource-map.rdf")),
        )
    ...
    def test_every_member_records_its_package_relative_location(self):
        ...
        locations = [
            (node.text or "") for node in knb._find_all_local(ore, "atLocation")
        ]
        expected = [
            obj["path"]
        ...
        # This is what makes the expanded representation reconstructible: a
        # consumer with the flat DataONE objects can rebuild the directory
        # tree. Without it the objects are named but unplaceable.
        self.assertEqual(sorted(locations), sorted(expected))
    ...
    def test_expanded_artifacts_are_eml_documented_entities(self):
        ...
        names = [
            (node.text or "")
            for entity in knb._find_all_local(eml, "otherEntity")
            for node in knb._find_all_local(entity, "objectName")
        ]
        artifacts = [
            obj["path"]
            for obj in result["manifest"]["objects"]
            if obj["role"] == "sdp_artifact"
        ]
        self.assertEqual(sorted(names), sorted(artifacts))
        # A CSV is not a container: declaring compressionMethod would tell a
        # consumer to unpack it.
        self.assertEqual(
            knb._find_all_local(eml, "compressionMethod"), []
        )

    def test_an_in_package_artifact_symlink_is_refused(self):
        sdp = _expanded_sdp(self)
        canonical = os.path.join(sdp, "metadata", "dataset.csv")
        target = os.path.join(sdp, "metadata", "dataset-target.csv")
    ...
    def test_only_a_symlinked_package_root_is_refused(self):
        sdp = _expanded_sdp(self)
        link_parent = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, link_parent, True)
        linked = os.path.join(link_parent, "linked-sdp")
        try:
            os.symlink(sdp, linked)
        except OSError:  # pragma: no cover - filesystem without symlinks
            self.skipTest("Filesystem does not permit symlink creation")
        for candidate in (linked, linked + os.sep):
        ...
        # commonly spelled through the harmless /var -> /private/var alias, and
...
class KnbFullExpandedInventoryTests(unittest.TestCase):
    ...
    def setUp(self):
        self.expected = _expected()["full_expanded"]
    ...
    def test_object_order_pids_and_bytes_match_r(self):
        ...
        self.assertEqual(result["package_id"], self.expected["package_id"])
        by_key = {
            (obj["role"], obj["path"]): obj for obj in self.expected["objects"]
        }
        for obj in planned:
            if obj["role"] == "resource_map":
                continue  # formatting-only byte difference, checked below
            source = by_key[(obj["role"], obj["path"])]
            with self.subTest(path=obj["path"]):
                self.assertEqual(obj["pid"], source["pid"])
                self.assertEqual(obj["sha256"], source["sha256"])
    ...
    def test_ore_and_eml_are_structurally_identical_to_r(self):
        sdp, result = self._plan()
        self.assertEqual(
            _canonical_file(str(result["resource_map_path"])),
            _canonical_file(os.path.join(_R_DIR, "full", "resource-map.rdf")),
        )
        self.assertEqual(
            _canonical_file(os.path.join(sdp, "metadata", "eml.xml")),
    ...
    def test_the_reproducibility_manifest_stays_valid_after_planning(self):
        ...
        sdp, _ = self._plan()
        self.assertTrue(validate_sdp_reproducibility_manifest(sdp))

    def test_a_partial_observation_structure_pair_is_refused(self):
        sdp = _fixture_sdp(self, "sdp-full")
        os.unlink(
            os.path.join(
                sdp, "metadata", "structure", "observation_components.csv"
            )
        )
        with self.assertRaises(ValueError) as caught:
            publish_sdp_to_knb(
    ...
    def test_the_mapping_must_bind_the_manifest_declared_ledger(self):
        import yaml
        ...
        with open(mapping_path, encoding="utf-8") as handle:
            mapping = yaml.safe_load(handle)
        mapping["semantic_review"]["path"] = "reviewed_semantic_selections.csv"
        with open(mapping_path, "w", encoding="utf-8") as handle:
...
@_REQUIRES_EXTRA
class KnbPublicationFileModeTests(unittest.TestCase):
    """Publication artifacts must land at the umask default, not 0600.

    ``_atomic_write_raw`` inlined ``mkstemp`` + ``os.replace``, and both of
    those preserve mkstemp's hard-coded 0600 -- so the recovery manifest, the
    resource map and the SDP archive shipped owner-only. That is exactly the
    defect ``atomic_io`` was written to prevent (PARITY.md row 24); this pins
    the KNB path to the shared writer.
    """

    def _plain_write_mode(self, directory: str) -> int:
        control = os.path.join(directory, "control.txt")
        with open(control, "w", encoding="utf-8") as handle:
            handle.write("x")
        mode = stat.S_IMODE(os.stat(control).st_mode)
        os.unlink(control)
        return mode

    def test_publication_artifacts_match_a_plain_write(self):
        for umask_value in (0o022, 0o002):
            with self.subTest(umask=oct(umask_value)):
                ...
                try:
                    ...
                    for artifact in (
                        ...
                        result["resource_map_path"],
                        result["sdp_archive_path"],
                    ):
                        self.assertEqual(
                            stat.S_IMODE(os.stat(str(artifact)).st_mode),
                            expected,
                            msg=str(artifact),
                        )
                        self.assertTrue(
