...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1099
passed / 1 skipped with the extras installed, 958 / 142 with core dependencies
only (2026-09-25, measured locally on hub B-188's branch with `main` `ed5e22e`
merged in, under Python 3.11.15, pytest 9.1.1 and pandas 3.0.6, on a machine
where `Rscript` is on `PATH` and `/tmp/metasalmon-lib` exists; 1075 / 1 and
934 / 142 on that branch with `main` `2405df2` merged in, 1065 / 1 and 924 / 142
for B-188 before those merges, 1056 / 1 and 915 / 142 for B-215 on `9577f35`,
which has `main` `2405df2` merged in, 1032 / 1 and 891 / 142 for B-216 with
