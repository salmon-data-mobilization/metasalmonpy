...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1107
passed / 1 skipped with the extras installed, 966 / 142 with core dependencies
only (2026-09-25, measured locally on hub B-188's branch with `main` `25dc7f3`
merged in, under Python 3.11.15, pytest 9.1.1 and pandas 3.0.6, on a machine
where `Rscript` is on `PATH` and `/tmp/metasalmon-lib` exists; 1064 / 1 and
923 / 142 for B-212 on `351fed6`, branched from `main` `70fa8fd`, 1104 / 1 and
963 / 142 on B-188's branch with `main` `70fa8fd` merged in, 1061 / 1 and
