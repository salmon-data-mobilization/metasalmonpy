...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1175
passed / 2 skipped with the extras installed, 1033 / 144 with core dependencies
only (2026-09-25, measured locally for hub B-274 on `e671772`, its merge of
`main` `cae3d83`, and 1166 / 2 and 1024 / 144 on `main` `cae3d83` before it;
1154 / 2 and 1012 / 144 for B-274 on `188769b`, branched from `main` `e089b86`,
and 1145 / 2 and 1003 / 144 on `main` `e089b86` before it, as for B-370 on
`6881aa0`, its merge of `main` `6700062`, whose tests `main` `e089b86` merged
unchanged; each under Python 3.11.15, pytest 9.1.1 and pandas 3.0.6, on a
...
`python -m pytest -q` and bare `pytest -q`; 1166 / 2 and 1024 / 144 for B-364
on `2fbde1e`, its merge of `main` `e089b86`, under Python 3.13.11, pytest 9.1.1
and pandas 3.0.5, in a worktree of a full clone with `METASALMON_PATH` unset, by
`python -m pytest -q` in isolated environments built from `.[test]` and
`.[test,eml,context]`; 1161 / 2 and 1020 / 143 for B-364 on `6c91919`, its
merge of `main` `380a7a4`; 1105 / 1 and 964 / 142 for B-364 on `ac96fb7`,
branched from `main` `ba1b54a`; and, under the same conditions as B-274's,
1143 / 2 and 1002 / 143 for B-370 on `dd43cd2`, its merge of `main` `056fccc`,
