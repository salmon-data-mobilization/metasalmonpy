...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1052
passed / 1 skipped with the extras installed, 911 / 142 with core dependencies
only (2026-09-25, measured locally for hub B-215 on `73314b7`, under Python
3.11.15, pytest 9.1.1 and pandas 3.0.6, on a machine where `Rscript` is on
`PATH` and `/tmp/metasalmon-lib` exists; 1030 / 1 and 889 / 142 for B-220 on
`acf243e` earlier that day, 1022 / 1 and 881 / 142 for B-241 on
the tree of its head `5b83c27` on 2026-09-24, 1012 / 1 and
