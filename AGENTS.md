...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1102
passed / 1 skipped with the extras installed, 961 / 142 with core dependencies
only (2026-09-25, measured locally for hub B-234 on `f872f51`, branched from
...
under `python -m pytest -q` and bare `pytest -q`; 1088 / 1 and 947 / 142 for
B-243 on `c4bdfb9`, branched from the same `main` and measured the same way;
1084 / 1 and 943 / 142 on `main` `ba1b54a` before both, measured the same four
ways; 1066 / 1 and 925 / 142
