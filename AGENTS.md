...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1141
passed / 2 skipped with the extras installed, 1000 / 143 with core dependencies
only (2026-09-25, measured locally for hub B-244 on `4722f17`, its merge of
...
`python -m pytest -q` and bare `pytest -q`; 1142 / 2 and 1000 / 144 for B-245
on `5e3c0c1`, branched from the same `main`, measured the same four ways;
