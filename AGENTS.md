...
dependency configurations, and CI runs both (see *Dependency boundaries*): 1104
passed / 1 skipped with the extras installed, 963 / 142 with core dependencies
only (2026-09-25, measured locally on hub B-188's branch with `main` `70fa8fd`
merged in, under Python 3.11.15, pytest 9.1.1 and pandas 3.0.6, on a machine
where `Rscript` is on `PATH` and `/tmp/metasalmon-lib` exists; 1061 / 1 and
